...
class AuthController extends Controller
{
    ...
    public function uploadImage(Request $request){
        $validator = Validator::make($request->all(), [
            'image' => 'required|image:jpeg,png,jpg,gif,svg|max:15728640'
        ]);
        if ($validator->fails()) {
            $response = [
                'success' => false,
    ...
    public function changePassword(Request $request) {
        ...
        if (!(new BcryptHasher)->check($request->input('old_password'), $user->password)) {
            return response()->json(['error' => 'Please insert correctly old password'], 401);
        }
    ...
    public function getPartnerHome(Request $request){
        $validator = Validator::make($request->all(), [
            'id'=>'required'
        ]);
        $productsOrders = ProductOrders::where('salon_id', $request->id)->limit(10)->orderBy('id','desc')->get();
        foreach($productsOrders as $loop){
            $loop->userInfo = User::where('id',$loop->uid)->first();
            if($loop->freelancer_id == 0){
                $loop->salonInfo = Salon::where('uid',$loop->salon_id)->first();
        ...
        $appointments = Appointments::where('salon_id', $request->id)->limit(10)->orderBy('id','desc')->get();
        foreach($appointments as $loop){
            $loop->userInfo = User::where('id',$loop->uid)->first();
            if($loop->freelancer_id == 0){
                $loop->salonInfo = Salon::where('uid',$loop->salon_id)->first();
            ...
            $services = json_decode($loop->items);
            $serviceList = $services->services;
            $serviceArray = [];
            foreach ($serviceList as $service) {
                $serviceItem = Services::where('id', $service->id)->first();
                array_push($serviceArray, $serviceItem);
            }
            $loop->services = $serviceArray;
        }
        ...
        $todayEarning = Appointments::select(\DB::raw("sum(grand_total) as total"), \DB::raw("DATE_FORMAT(save_date,'%h:%m') as day_name"), \DB::raw("DATE_FORMAT(save_date,'%h:%m') as day"))
        ->whereDate('save_date',Carbon::today())
        ->where('salon_id', $request->id)
        ->where('status', 4)
        ->groupBy('day_name','day')
        ->orderBy('day')
        ->first();
        $weekEarning = Appointments::select(\DB::raw("sum(grand_total) as total"), \DB::raw("DATE(save_date) as day_name"), \DB::raw("DATE(save_date) as day"))
            ->whereBetween('save_date', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])
            ->where('salon_id', $request->id)
            ->where('status', 4)
            ->groupBy('day_name','day')
            ->orderBy('day')
            ->first();
        $monthEarning = Appointments::select(\DB::raw("sum(grand_total) as total"), \DB::raw("DATE(save_date) as day_name"), \DB::raw("DATE(save_date) as day"))
            ->whereMonth('save_date', Carbon::now()->month)
            ->where('salon_id', $request->id)
            ->where('status', 4)
            ->groupBy('day_name','day')
        ...
        $response = [
            ...
            'productsOrders'=>$productsOrders,
            'appointments'=>$appointments,
            'total_orders'=>ProductOrders::where('salon_id', $request->id)->count(),
            'total_appointments'=>Appointments::where('salon_id', $request->id)->count(),
            'today_earning' => $todayEarning,
            'week_earning' => $weekEarning,
