...
class ProductsController extends Controller
{
    ...
    public function getByPartnerId(Request $request){
        $validator = Validator::make($request->all(), [
            'id' => 'required',
        ]);
        if ($validator->fails()) {
        ...
        foreach($data as $loop){
            if($loop && $loop->cate_id && $loop->cate_id !=null){
                $cats = ProductCategory::where('id',$loop->cate_id)->first();
                $loop->category = $cats;
            }
            if($loop && $loop->cate_id && $loop->cate_id !=null){
                $sub_cats = ProductSubCategory::where('id',$loop->sub_cate_id)->first();
                $loop->sub_category = $sub_cats;
            }
            if($loop && $loop->uid && $loop->uid !=null){
                $loop->user = User::find($loop->freelacer_id);
            }
        }
    ...
    public function topProducts(Request $request){
        ...
        $products = Products::WhereIn('freelacer_id',$uidArray)->limit(10)->get();
        $response = [
            'products'=>$products,
            'success' => true,
            'status' => 200,
