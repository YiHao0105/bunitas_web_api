...
class AppointmentsController extends Controller
{
    ...
    public function getInfo(Request $request){
        ...
        $services = json_decode($data->items);
        $serviceList = $services->services;
        $serviceArray = [];
        foreach ($serviceList as $loop) {
            $serviceItem = Services::where('id', $loop->id)->first();
    ...
    public function orderInvoice(Request $request){
        ...
        try {
            ...
            $distance = 0;
            if($data->appointments_to == 1){
                foreach ($serviceArray as $loop) {
                    $distance += $serviceArray->distance;
                }
            }
            $discount = $data->discount;

            $data->appoint_subtotal = $price + $distance - ($price * $discount) / 100;
            $data->appoint_deposit_fee = $price * $general->deposit_now / 100;
            $data->appoint_processing_fee = $price * $general->deposit_now / 100 * $general->processing_fee / 100;
            $data->appoint_distance = $distance;
    ...
    public function getAppointmentsForSalon(Request $request) {
        $validator = Validator::make($request->all(), [
            'id'     => 'required',
            'from'     => 'required',
            'to'     => 'required',
        ...
        $from = date($request->from);
        $to = date($request->to);
        $data = Appointments::where('salon_id', $request->id)->whereBetween('save_date',[$from, $to])->get();
        foreach($data as $loop){
            $loop->salonInfo = Salon::where('uid',$loop->salon_id)->first();
            $loop->userInfo = User::select('id','first_name','last_name','cover')->where('id',$loop->uid)->first();
        }
